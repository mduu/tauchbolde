...
namespace Tauchbolde.Tests.Application.UseCases.Logbook
{
    public class ListAllLogbookEntriesHandlerTests
    {
        ...
        {
            var result = await handler.Handle(new ListAllLogbookEntries(false), CancellationToken.None);
        ...
        {
            var result = await handler.Handle(new ListAllLogbookEntries(true), CancellationToken.None);
