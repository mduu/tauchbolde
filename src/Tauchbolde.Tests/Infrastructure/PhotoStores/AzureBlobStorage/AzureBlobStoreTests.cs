...
namespace Tauchbolde.Tests.Infrastructure.PhotoStores.AzureBlobStorage
{
    public class AzureBlobStoreTests
    {
        ...
        public async Task Test_AddPhoto()
        {
            ...
            A.CallTo(() => logger.Log<object>(LogLevel.Error, A<EventId>._, A<object>._, A<Exception>._, A<Func<object, Exception, string>>._))
                .MustNotHaveHappened();
        }
        
        private AzureBlobStore CreateStore(IAzureBlobStoreConfiguration configuration = null)
        {
